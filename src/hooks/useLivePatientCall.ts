...
import { playPcm16Base64, playSpeechBase64, primeSpeechOutput, speakText, stopSpeaking } from '../lib/speech';
...
interface UseLiveVoiceCallOptions {
  ...
  /** Use recorder audio even when browser speech recognition is available. */
  forceAudioRecording?: boolean;
  disabled?: boolean;
...
// Keep short Egyptian questions intact, but do not append several seconds of
// room noise after the student has finished speaking.
const SILENCE_MS = IS_MOBILE ? 1600 : 1250;
const MIN_SPEECH_MS = IS_MOBILE ? 380 : 400;
const MAX_RECORDING_MS = IS_MOBILE ? 9000 : 8000;
const NO_SPEECH_TIMEOUT_MS = IS_MOBILE ? 5500 : 4500;
...
/**
 * Short questions often begin immediately after the student starts the call.
 * During the VAD noise-floor warmup, accept only clearly audible speech on the
 * forced recorder path; otherwise that opening can be absorbed as "noise" and
 * never sent to STT.
 */
const EARLY_SPEECH_RMS_THRESHOLD = 0.01;
...
function classifyTurnError(err: unknown): {
  ...
  const response = (err as {
    response?: { status?: number; data?: { code?: string; error?: string; stage?: string } };
  })?.response;
  const status = response?.status;
  const serverCode = response?.data?.code;
  if (serverCode === 'openrouter-transcription-hallucination') {
    return { code: 'unintelligible-audio', status, fatal: true, softRetry: false };
  }
  if (
    response?.data?.stage === 'transcription' ||
    serverCode === 'openrouter-transcription-network-failed'
  ) {
    return { code: 'stt-provider-unavailable', status, fatal: false, softRetry: false };
  }
  if (code === 'tts-failed' || response?.data?.stage === 'audio-generation') {
    return { code: 'tts-failed', status, fatal: false, softRetry: false };
  }
  if (
...
export function useLiveVoiceCall({
  ...
  const playVoiceTurnAudio = useCallback(async (audioBase64: string, audioFormat?: string) => {
    if (!audioBase64 || !liveRef.current) return;
    speakingRef.current = true;
    setSpeaking(true);
    setStreamMuted(streamRef.current, true);
    try {
      if (audioFormat === 'pcm16') await playPcm16Base64(audioBase64);
      else {
        const mimeType = audioFormat === 'mp3'
          ? 'audio/mpeg'
          : `audio/${audioFormat || 'wav'}`;
        await playSpeechBase64(audioBase64, mimeType);
      }
    } catch {
  ...
  const processTextTurn = useCallback(
    async (transcript: string) => {
      ...
      try {
        if (turn) {
          ...
          softRetryCountRef.current = 0;
          turn.onTurn?.(result);
          if (result.audioBase64) await playVoiceTurnAudio(result.audioBase64, result.audioFormat);
          else await playReply(result.replyMessage.content);
          if (typeof result.latencyMs === 'number') console.info('[voice/openrouter-audio] request-to-complete ms', result.latencyMs, 'first response ms', result.firstResponseMs);
    ...
    [playReply, playVoiceTurnAudio],
  );

  const processTurn = useCallback(
    async (blob: Blob, recordingMeta?: { durationMs: number; speechDurationMs: number }) => {
      if (processingTurnRef.current) return false;
      processingTurnRef.current = true;
      const turn = voiceTurnRef.current;
      ...
      try {
        if (turn) {
          ...
          if (typeof result.latencyMs === 'number') console.info('[voice/openrouter-audio] recording-finished-to-complete ms', result.latencyMs, 'first response ms', result.firstResponseMs);
          return !!result.transcript?.trim();
        }
  ...
  const listenOnce = useCallback(async () => {
    ...
    if (preferWhisperForEnglish && shouldUseBrowserStt() && !forceAudioRecording && !forceServerSttRef.current && !IS_MOBILE) {
    ...
    try {
      ...
      recorder.onstop = async () => {
        ...
        const detectedSpeechDurationMs = speechStartedAtRef.current && lastSpeechAt
          ? Math.max(0, lastSpeechAt - speechStartedAtRef.current)
          : 0;

        console.info('[voice/openrouter-audio][capture]', {
          mimeType: blob.type,
          byteSize: blob.size,
          recordingDurationMs: elapsed,
          speechDurationMs: detectedSpeechDurationMs,
          speechDetected: Boolean(speechStartedAtRef.current),
        });
        ...
        if (
          ...
          if (noSpeechRetryCountRef.current >= MAX_SOFT_RETRIES) {
      ...
      const monitor = () => {
        ...
        if (speechStartedAtRef.current) {
          ...
          const earlyRecorderSpeech =
            forceAudioRecording && now < noiseWarmupUntil && rms >= EARLY_SPEECH_RMS_THRESHOLD;
          if ((now >= noiseWarmupUntil && rms >= speechGate) || earlyRecorderSpeech) {
            speechFrames += 1;
