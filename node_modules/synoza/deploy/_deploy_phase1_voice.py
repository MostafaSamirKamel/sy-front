"""Deploy Phase 1 voice/AI fixes: client dist + server dist."""
import shutil
import sys
import tarfile
...
STAGE = ROOT / "deploy" / "_phase1_voice_stage"
TAR = ROOT / "deploy" / "_phase1_voice_hotfix.tar.gz"
...
# Also ship updated server source files used by tsx seed paths if present
for rel in [
    "server/src/services/aiService.ts",
    "server/src/services/examinerVivaService.ts",
...
print(">>> uploading Phase 1 hotfix")
client = paramiko.SSHClient()
client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
client.connect(HOST, port=PORT, username=USER, password=PASSWORD, timeout=30)
sftp = client.open_sftp()
remote = "/tmp/_phase1_voice_hotfix.tar.gz"
...
grep -n 'getVivaClosing\\|بالتوفيق\\|timeoutMs\\|CHAT_EXAMINER_MAX_TOKENS' {APP}/server/dist/services/examinerVivaService.js {APP}/server/dist/services/aiService.js 2>/dev/null | head -20
