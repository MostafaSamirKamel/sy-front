...
(async () => {
  ...
  let pe = row.physicalExam || '';
  let parsed;
  try { parsed = JSON.parse(pe); } catch { parsed = null; }
  if (parsed && typeof parsed === 'object') {
    console.log(JSON.stringify({
      case: row.titleEn,
      inspection: parsed.inspection || '',
      palpation: (parsed.palpation || '').slice(0, 120),
      keys: Object.keys(parsed),
    }, null, 2));
  } else {
    console.log(JSON.stringify({ case: row.titleEn, physicalExam: pe }, null, 2));
  }
  await p.$disconnect();
