...
try {
  ...
  console.log('ascites', ascites?.id);
  const sessionCount = await p.session.count({ where: { caseId: ascites?.id } });
  ...
  const canned = await p.message.findMany({
    where: { content: { contains: 'شكوى' } },
    take: 10,
    orderBy: { createdAt: 'desc' },
    include: { session: { include: { case: { select: { titleEn: true } } } } },
  });
  console.log('SHAKWA_MSGS', canned.length);
...
'''
sftp = c.open_sftp()
with sftp.file(f"{APP}/scripts/_diag_msgs3.ts", 'w') as f: f.write(script)
sftp.close()
_, o, e = c.exec_command(f'cd {APP}; npx tsx scripts/_diag_msgs3.ts 2>&1', timeout=120)
print(o.read().decode())
c.close()
