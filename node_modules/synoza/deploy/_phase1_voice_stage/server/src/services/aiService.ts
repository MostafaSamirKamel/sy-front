...
import { parseStationConfig, resolveManeuverOpeningMessage } from '../lib/stationConfig.js';
...
const CHAT_PATIENT_MAX_TOKENS = 96;
const CHAT_EXAMINER_MAX_TOKENS = 80;

function chatContextWindow(history: { role: string; content: string }[], maxMessages: number) {
  return contextWindow(history, Math.min(maxMessages, CHAT_CONTEXT_CAP));
}
...
export async function getPatientResponse(
  ...
  const promptHistory = chatContextWindow(history, settings.maxContextMessages);
  ...
  const maxTokens = voiceTurn
    ? Math.min(CHAT_PATIENT_MAX_TOKENS, 64)
