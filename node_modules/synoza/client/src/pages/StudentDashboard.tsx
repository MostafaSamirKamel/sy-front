...
export default function StudentDashboard() {
  ...
  const randomSectionsForBoard = useMemo(
    () => sectionOptions.filter((option) => option.id === selectedBoardId || subCategories.some((sub) => sub.id === option.id)),
    [sectionOptions, selectedBoardId, subCategories],
  );
  ...
  const selectBoard = (board: CategoryNode) => {
    ...
    const scopedSections = sectionOptions
      .filter((option) => option.id === board.id || board.children.some((child) => child.id === option.id))
      .map((option) => option.id);
    setRandomSectionIds(scopedSections.length ? [scopedSections[0]] : []);
    setSearch('');
  };
  ...
  const startRandomCase = async (categoryIds?: string[]) => {
    ...
    try {
      const randomRes = await api.get('/student/random-case', {
        params: {
          ...(ids.length ? { categoryId: ids.join(',') } : {}),
          ...(selectedBoardId ? { categoryRootId: selectedBoardId } : {}),
        },
      });
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      {entitlements?.isFree ? (
        <RandomCasePreview entitlements={entitlements} categoryRootId={selectedBoardId ?? undefined} />
      ) : entitlements && sectionOptions.length > 0 ? (
        <BlindMockOsceCard
          sections={randomSectionsForBoard.map((o) => ({
            id: o.id,
      ...
      <section className="space-y-5">
        ...
        {!search.trim() && subCategories.length > 0 && (
          <div className="max-w-full overflow-x-auto overscroll-x-contain pb-2 sm:overflow-visible" aria-label="System categories">
              ...
              {subCategories.map((sub) => {
                ...
                return (
                  <button
                    ...
                    className={`inline-flex shrink-0 items-center gap-2 px-4 py-2 rounded-full text-xs font-semibold border transition-all ${
                      selected
